...
- 每局開始時，隨機產生桌上玩家數 $N$，其中 $2 \le N \le 9$。
- 恰好 1 位為人類玩家（固定座位或隨機座位，見「待決項目」）。
- 其他 $N-1$ 位為 AI。
- 未入局的座位顯示為空位。
...
- `ante`（預設：0；可開關）。
- `blindIncreasePolicy`：盲注是否隨時間/手數提升（單機可先不做，或提供「固定盲注」為預設）。
...
1. 生成本局玩家數 $N \in [2,9]$，建立玩家列表（1 真人 + $N-1$ AI）。
2. 指派座位（Seat 0..8）與 Button（依上一局輪轉或本局隨機；見待決）。
3. 扣除 Ante（若啟用）。
4. 扣除盲注（SB/BB）。
5. 洗牌、發手牌。
6. 進行 Preflop 下注。
...
9. 若未提前結束：發 River → 下注。
10. 攤牌 / 結算（含邊池）。
11. 更新籌碼、淘汰（stack=0）處理、Button 輪轉。
12. 顯示結果並進入下一局或返回主選單。
...
- 平分底池時若有餘數 1（或多）籌碼，依座位順序發放：
	- 建議採用「最靠近 Button 的玩家優先」或「從 SB 起順時針」其一（見待決）。
- 規則需一致且可測。
...
- 一局結束後若有人 stack=0：
	- 可採「淘汰並在下一局重新補滿」或「淘汰後離桌」策略（見待決；第一版建議直接補滿以確保每局仍可 2–9 人隨機）。
...
- 每局開始時，桌上人數必為 2–9 隨機，且恰好 1 位人類玩家，其餘皆 AI。
- 能完整跑完以下情境且不崩潰、不違規：
	- 正常四街下注到攤牌。
	- Preflop 或任一街全員棄牌結束。
...
為避免規則分歧，以下請你選定（不選則採預設）：

1. 人類玩家座位：
	 - A) 固定在 Seat 0（預設）
	 - B) 每局隨機座位
2. Button 指派：
	 - A) 第一局隨機、之後每局順時針輪轉（預設）
	 - B) 每局隨機
3. Odd chip 規則：
	 - A) 由 Button 左手邊起順時針給餘籌碼（預設）
	 - B) 由 SB 起順時針
4. 盲注是否升級：
	 - A) 固定盲注（預設）
	 - B) 每 X 手/每 Y 分鐘提升
5. 破產處理（stack=0）：
	 - A) 下一局自動補滿起始籌碼（偏單機遊樂；預設）
	 - B) 淘汰離桌，直到不足 2 人時重新開桌
