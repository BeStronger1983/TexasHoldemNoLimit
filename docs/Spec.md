...
- Street：下注輪次（Preflop / Flop / Turn / River）。
	- 一個 Street 代表「同一輪的下注行動區間」；在此區間內，玩家依序完成一次（或多次因加注而回到前面）行動，直到該輪結束。
	- Street 與公共牌的關係：
		- Preflop：沒有公共牌，發完手牌後進入下注。
		- Flop / Turn / River：在該 Street 開始前會先發出對應公共牌（且依設定可能先燒牌），然後進入下注。
	- 各 Street 詳解（快速理解版）：
		- Preflop：
			- 公共牌數：0。
			- 典型流程：盲注/ante（若啟用）已投入後 → 發每位在局玩家 2 張手牌 → 進入第一輪下注。
			- 目標：在未揭示公共牌的情況下，透過下注決定是否繼續看 Flop，或在此就以棄牌/全下等方式結束。
		- Flop：
			- 公共牌數：3（Flop 結束後桌面共有 3 張公共牌）。
			- 典型流程：燒牌（若啟用）→ 發 3 張公共牌 → 進入第二輪下注。
			- 意義：大量資訊被揭露，手牌通常從「猜測」進入「形成牌型/抽牌」的階段。
		- Turn：
			- 公共牌數：4（Turn 結束後桌面共有 4 張公共牌）。
			- 典型流程：燒牌（若啟用）→ 發第 4 張公共牌（Turn）→ 進入第三輪下注。
			- 意義：抽牌剩餘一張牌可改善（或失敗），常影響有效籌碼與底池大小的決策。
		- River：
			- 公共牌數：5（River 結束後桌面共有 5 張公共牌）。
			- 典型流程：燒牌（若啟用）→ 發第 5 張公共牌（River）→ 進入最後一輪下注。
			- 意義：公共牌已完全揭示；若仍有 2 位以上玩家未棄牌，通常在此輪結束後進入攤牌/結算。
	- 行動順序（起始行動者）會隨 Street 改變：Preflop 從 UTG 開始；Postflop（Flop/Turn/River）從 BTN 左手邊第一位仍在局玩家開始（詳見 5.5）。
	- Street 的結束條件：見 5.6（例如只剩 1 人未棄牌、或所有仍在局且未 All-in 的玩家都已對齊當前最高下注額並完成回應）。
	- 下注投入的記錄通常分為：
		- `contributionThisStreet`：用於本輪對齊/跟注判定（每個 Street 重新計算）。
		- `contributionThisHand`：用於整手牌的底池/邊池切分（跨 Street 累加；見 5.7）。
- Button（BTN）：莊家位。
